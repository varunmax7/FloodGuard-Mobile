...
| p3 | 2026-06-20 | Ingest pipeline: ECMWF (hourly), AWS (15-min), Radar (10-min), Bias recompute (hourly). All tasks mock-mode by default (INGEST_MOCK=True). ForecastStage + IngestLog models added. RadarFrame.anomaly field added. Beat schedule defined in celery.py. | Phase 3 |
| p4 | 2026-06-20 | Risk engine (§3.2): hazard bucketing, 4×3 risk matrix, bias correction, confidence scoring. recompute_all_risk task. /risk/hexes (GeoJSON+bbox+GIST), /risk/location (hourly strip+plain-text), /risk/overview (city aggregates+hotspots), /radar/frames (intensity legend). 60s cache on all read endpoints. | Phase 4 |
